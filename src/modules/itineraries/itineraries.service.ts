...
export class ItinerariesService {
  constructor(
    ...
    // Sub-services
    private readonly hotspotService: ItineraryHotspotService,
    private readonly activityService: ItineraryActivityService,
    private readonly hotelService: ItineraryHotelService,
    private readonly vehicleService: ItineraryVehicleService,
    private readonly confirmationService: ItineraryConfirmationService,
    private readonly queryService: ItineraryQueryService,
  ) {}
  ...
  async deleteHotspot(planId: number, routeId: number, hotspotId: number) {
    return await this.hotspotService.deleteHotspot(planId, routeId, hotspotId);
  }
  ...
  async getAvailableActivities(routeId: number) {
    return await this.activityService.getAvailableActivities(routeId);
  }

  /**
   * Add an activity to a hotspot in the itinerary
   */
  async addActivity(data: { planId: number; routeId: number; activityId: number }) {
    return await this.activityService.addActivity(data);
  }
  ...
  async deleteActivity(planId: number, routeId: number, activityId: number) {
    return await this.activityService.deleteActivity(planId, routeId, activityId);
  }
  ...
  async getAvailableHotspots(routeId: number) {
    return await this.hotspotService.getAvailableHotspots(routeId);
  }
  ...
  async addHotspot(data: { planId: number; routeId: number; hotspotId: number }) {
    return await this.hotspotService.addHotspot(data);
  }
  ...
  async previewAddHotspot(data: { planId: number; routeId: number; hotspotId: number }) {
    return await this.hotspotService.previewAddHotspot(data);
  }
  ...
  async getAvailableHotels(routeId: number) {
    return await this.hotelService.getAvailableHotels(routeId);
  }
  ...
  async selectHotel(data: {
    ...
    return await this.hotelService.selectHotel(data);
  }
  ...
  async bulkSaveHotels(planId: number, hotels: any[]) {
    return await this.hotelService.bulkSaveHotels(planId, hotels);
  }
  ...
  async selectVehicleVendor(data: {
    ...
    return await this.vehicleService.selectVehicleVendor(data);
  }

  async getPlanForEdit(planId: number) {
    return await this.queryService.getPlanForEdit(planId);
  }

  async getCustomerInfoForm(planId: number) {
    return await this.queryService.getCustomerInfoForm(planId);
  }

  async checkWalletBalance(agentId: number) {
    return await this.queryService.checkWalletBalance(agentId);
  }

  async getAgentWalletBalance(agentId: number) {
    return await this.queryService.getAgentWalletBalance(agentId);
  }

  async confirmQuotation(dto: ConfirmQuotationDto) {
    // Delegate to confirmation service
    const result = await this.confirmationService.confirmQuotation(dto);
    
    // If hotel bookings are provided, process them outside the transaction
    if (dto.hotel_bookings && dto.hotel_bookings.length > 0) {
      return await this.confirmationService.processConfirmationWithTboBookings(
        result,
        dto,
        this.tboHotelBooking,
        this.resavenueHotelBooking,
        this.hobseHotelBooking,
        dto.endUserIp || '192.168.1.1'
      );
    }
  ...
  /**
   * Process hotel bookings after confirmation (delegated to confirmation service)
   */
  ...
  async cancelItinerary(dto: CancelItineraryDto) {
    return await this.confirmationService.cancelItinerary(dto);
  }

  async getAgentsForFilter(req: any) {
    return await this.queryService.getAgentsForFilter(req);
  }
