...
const { TimelinePrefetcher } = require('./src/modules/itineraries/engines/helpers/timeline.prefetch');
const { HotspotSelector } = require('./src/modules/itineraries/engines/helpers/timeline.hotspot-selector');
...
async function main() {
  ...
  const ctx = await prefetcher.fetchAll(prisma, planId);
  ...
  for (const route of ctx.routes) {
    ...
    const selected = await selector.selectForRoute(prisma, ctx.plan, route, ctx, []);
